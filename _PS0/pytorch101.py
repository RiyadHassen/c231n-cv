...
def create_sample_tensor() -> Tensor:
    ...
    two entries: entry (0, 1) which is set to 10 and entry (1, 0) which is set to
    100.
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if stop - start >= 10:
      x = torch.arange(10,stop,10).to(torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # .to() casts the data type of the argument to the datatype of the argument
    x[:2,:6] = torch.tensor([[0,1,2,2,2,2],[0,1,2,2,2,2]]).to(x)
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    Returns:
        z: Tensor of shape (B, N, P) where z[i] of shape (N, P) is the result
            ...
            shape (M, P). The output z should have the same dtype as x.
    """
    z = torch.rand((x.shape[0],x.shape[1],y.shape[2]))
    ###########################################################################
    #                      TODO: Implement this function                      #
    ###########################################################################
    # Replace "pass" statement with your code
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    #calculate the mu for each col
    mu = (1 / int(x.shape[1])) * (y.sum(dim=0,keepdim=True))
    
    temp = (y - mu)**2 

    sigma = ((1 / (x.shape[0]-1)) *temp.sum(dim=0,keepdim=True)).sqrt()
    
    print(x.std(dim=0))
    
    y = (y - mu) / sigma 
    
    # we don't need m = torch.ones((1,x.shape[1])), right? we can use broadcasting 
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
